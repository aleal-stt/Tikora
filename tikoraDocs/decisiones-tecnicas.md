...
**Decisión:** **Vitest** como runner, con **`fast-check`** para _property-based testing_ en la lógica crítica (clasificación, transiciones de estado, validaciones). Tests por feature dentro de `__tests__/`.
...
**Decisión:** Frontend como **SPA** con **Vite + React 19 + TypeScript**, sirviendo estáticos. El backend solo expone API REST en `/api/v1`. La aplicación vive como app dentro del monorepo Nx en `apps/front/`.
...
**Decisión:** **Access token en memoria + refresh token en cookie `httpOnly`** emitida por el backend con `SameSite=Lax`, `Path=/api/v1/auth` y `Secure` activado solo en HTTPS (en dev local sobre HTTP queda desactivado). El refresh token **rota en cada uso**. La respuesta de login devuelve solo `{ accessToken, user }` en JSON y el refresh viaja exclusivamente en `Set-Cookie`. Interceptor de fetch en `lib/api-client.ts` maneja el refresh automático ante `401`. Logout multi-pestaña vía evento `storage`.
...
**Setup de origen para que la cookie funcione sin fricción:**

- En **dev**, el frontend corre con el proxy de Vite (`/api` → `http://localhost:3001`), de modo que las requests son **same-origin** desde el browser y la cookie viaja sin problemas de `SameSite`/CORS.
- En **prod** se sirve front y back detrás del mismo reverse proxy (nginx/caddy) compartiendo origen.
- La cookie usa `Path=/api/v1/auth` para que solo se envíe a los endpoints de auth, reduciendo su superficie de uso.
...
**Rotación del refresh token:** cada `POST /auth/refresh` exitoso emite un refresh nuevo y reemplaza la cookie. El refresh anterior queda inválido en backend. Si llega un refresh ya invalidado, se asume sesión comprometida: se invalida toda la cadena de refresh del usuario y se fuerza login.
...
## 25. Autenticación del stream SSE

**Decisión:** El cliente autentica la apertura del stream SSE con un **ticket de vida corta** (60–120 s, single-use, firmado, vinculado a `userId` y `tenantId`). El cliente lo solicita a `POST /api/v1/auth/sse-ticket` con su access token en `Authorization`, recibe el ticket y abre el stream con `new EventSource('/api/v1/notifications/stream?ticket={ticket}')`. El backend valida el ticket en el handshake, lo invalida y mantiene el stream abierto.

**Opciones evaluadas:**

| Opción                                                                           | Pros                                                                                                                 | Contras                                                                                                |
| -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Ticket corto vía endpoint dedicado (elegida)                                     | Compatible con `EventSource` estándar; expone solo un secreto efímero en el query string; revocable y de un solo uso | Un endpoint extra y un mecanismo nuevo de tickets                                                      |
| Pasar el access token largo en query param                                       | Trivial de implementar                                                                                               | El access token (15 min) queda expuesto en logs de proxy/server; explotación más sencilla si se filtra |
| Reemplazar `EventSource` por `fetch + ReadableStream` con header `Authorization` | Permite header de auth nativo                                                                                        | Más código de cliente; sin reconexión automática del browser; requiere mantener una abstracción        |
